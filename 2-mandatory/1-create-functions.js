...
function tidyUpString(array) {
  return array.map(element => element.replace(/\s/g,'').replace('/',''));
}
...
*/
//working on
function remove(array, ind) {
  return array.filter((_, i) => ind !== i)
}
// console.log(remove([' hjk ', ' hjkkk', 'hh/hh '], 2))
/*
Write a function that:
...
function formatPercentage(array) {
  return array.map(element => element.toFixed(2)<100? element.toString(element) : element.toString(100))
}
